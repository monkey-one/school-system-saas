...
<div class="space-y-6">
    ...
    <div class="lg:hidden bg-white rounded-xl border border-gray-100 p-4">
        <label class="text-xs font-semibold text-gray-400 uppercase tracking-wider block mb-2">Anak yang dipilih</label>
    ...
    <div class="bg-gradient-to-r from-navy-700 to-navy-600 rounded-2xl p-6 sm:p-8 text-white">
        <h2 class="text-xl sm:text-2xl font-heading font-bold mb-1">Halo, {{ $parent->name ?? 'Bapak/Ibu' }}! 👋</h2>
        <p class="text-white/60 text-sm">Memantau perkembangan <strong class="text-gold-400">{{ $selectedChild->full_name ?? '' }}</strong> — {{ $selectedChild->classroom->name ?? '' }}</p>
    </div>
    ...
    <div class="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        <div class="bg-white rounded-xl border border-gray-100 p-5">
            ...
            <div class="flex gap-4 mt-2 text-xs text-gray-400">
                <span>Hadir: {{ $attendanceSummary['hadir'] ?? 0 }}</span>
                <span>Sakit: {{ $attendanceSummary['sakit'] ?? 0 }}</span>
                <span>Izin: {{ $attendanceSummary['izin'] ?? 0 }}</span>
                <span>Alfa: {{ $attendanceSummary['alfa'] ?? 0 }}</span>
            </div>
        </div>
        ...
        <div class="bg-white rounded-xl border border-gray-100 p-5">
            <div class="flex items-center gap-3 mb-3">
                ...
                <span class="text-xs font-semibold text-gray-400 uppercase">SPP {{ now()->translatedFormat('F') }}</span>
            </div>
            @if($sppStatus['is_paid'] ?? false)
                <div class="text-2xl font-heading font-bold text-green-600">Lunas</div>
            @else
    ...
    <div class="grid lg:grid-cols-2 gap-6">
        ...
        <div class="bg-white rounded-xl border border-gray-100 p-6">
            ...
            <div class="space-y-3">
                ...
                @empty
                <div class="text-center py-8 text-gray-400 text-sm">Belum ada nilai</div>
                @endforelse
            </div>
        </div>
        ...
        <div class="bg-white rounded-xl border border-gray-100 p-6">
            ...
            <div class="flex items-center gap-3 p-3 bg-gray-50 rounded-lg mb-4">
                ...
                <div>
                    ...
                    <div class="text-xs text-gray-400">Wali Kelas {{ $selectedChild->classroom->name ?? '' }}</div>
                </div>
            </div>
            @endif
            ...
            <div class="space-y-3 max-h-64 overflow-y-auto mb-4">
                ...
                @empty
                <div class="text-center py-6 text-gray-400 text-sm">Belum ada pesan</div>
                @endforelse
            </div>
